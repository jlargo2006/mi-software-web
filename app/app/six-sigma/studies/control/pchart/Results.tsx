// app/app/six-sigma/studies/control/pchart/Results.tsx
"use client";
import React from "react";
import type { Data, Layout, Shape, Annotations } from "plotly.js";
import ResultChart from "../../../components/ResultChart";
import type { PChartResult, Violation } from "./types";
...
export default function PChartResults({ result }: { result: PChartResult }) {
  if (!result.ok) {
    return (
  ...
  // Los limites se dibujan escalonados: con tamanos de muestra desiguales
  // cada punto tiene el suyo, porque la sigma binomial va con 1/raiz(n).
  ...
  const traces: Data[] = [
    ...
    step(r.cl, GREEN, "solid"),
    {
      ...
      customdata: xs.map((i) => [r.d[i - 1], r.n[i - 1]]),
      hovertemplate:
        "Sample %{x}<br>p = %{y:.4f}<br>%{customdata[0]} of %{customdata[1]}<extra></extra>",
    } as unknown as Data,
    {
      x: fl,
      y: fl.map((i) => r.p[i - 1]),
    ...
    <div className="w-full space-y-4">
      ...
      <div>
        <p className="mb-1 text-xs font-semibold text-gray-800">
          Test Results for P Chart of {r.colName}
        </p>
      ...
      {r.flagged.length > 0 && (
        <div className="rounded-md border border-blue-300 bg-blue-50 px-4 py-3 text-sm text-blue-900">
          ...
          <p className="mt-1">
            ...
            different definition of defective. Those shift the proportion exactly
            like a real deterioration and cost nothing to rule out.
          </p>
        </div>
      )}
      ...
      {over && (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          ...
          <p className="mt-1">
            The binomial model assumes every unit in a subgroup has the same
            probability of being defective and that units are independent. When
            subgroups are large that assumption fails easily {"\u2014"} batches
            differ, shifts differ {"\u2014"} and the real variation exceeds
            {"\u00A0"}
            {"\u221A"}(p{"\u0305"}(1{"\u2212"}p{"\u0305"})/n). The limits are then
            too narrow and the chart signals on ordinary batch-to-batch noise. A
            Laney P{"\u2032"} chart, which widens the limits by this factor, is
            the standard remedy.
          </p>
        </div>
      )}
      ...
      {r.minNP < 5 && (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          ...
          <p className="mt-1">
            The smallest expected count is {fx(r.minNP, 1)}. The three-sigma
            limits come from a normal approximation to the binomial, which needs
            roughly n{"\u00B7"}p{"\u0305"} and n(1{"\u2212"}p{"\u0305"}) above 5.
            Below that the limits are noticeably wrong on the short side and the
            false alarm rate is not the nominal one. Larger samples, or an exact
            binomial chart, are the way out.
          </p>
        </div>
      )}
      ...
      {r.flagged.length === 0 && !over && !under && (
        <div className="rounded-md border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          ...
          <p className="mt-1">
            The proportion defective is stable at {fx(s0.pBar * 100, 2)}%. Stable
            is not the same as acceptable: a controlled process simply keeps
            producing the same rate, and only a deliberate change to the process
            will lower it.
          </p>
        </div>
      )}
      ...
      <p className="text-xs text-gray-500">
        ...
        {r.usedHistorical && ", historical proportion supplied"}. Dispersion
        ratio {fx(r.dispersion, 2)}.
        {r.omitted.length > 0 && (
          <>
            {" "}
