// app/app/six-sigma/studies/control/pchart/Controls.tsx
"use client";
...
export default function PChartControls({
  params,
  onChange,
  columns,
}: {
  params: PChartParams;
  onChange: (p: PChartParams) => void;
  columns: ColumnInfo[];
}) {
  const [tab, setTab] = useState<Tab>("data");
  const set = (patch: Partial<PChartParams>) => onChange({ ...params, ...patch });
      ...
      {tab === "data" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            The count is the number of <em>units</em> found defective, and it
            cannot exceed the sample size. If a unit can carry several defects
            and you are counting defects rather than units, the right chart is a
            U or C chart.
          </p>
        </div>
      )}
      ...
      {tab === "limits" && (
        <div className="space-y-3">
          ...
          <div className="border-t border-gray-200 pt-2">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Actual sizes give each point the limit it deserves. A single
              assumed size produces a flat, readable chart at the cost of being
              wrong for every subgroup that is not that size {"\u2014"} useful
              only when the sizes vary little.
            </p>
          </div>
        </div>
      )}
