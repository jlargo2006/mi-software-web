// app/app/six-sigma/studies/control/npchart/Controls.tsx
"use client";
...
export default function NPChartControls({
  params,
  onChange,
  columns,
}: {
  params: NPChartParams;
  onChange: (p: NPChartParams) => void;
  columns: ColumnInfo[];
}) {
  const [tab, setTab] = useState<Tab>("data");
  const set = (patch: Partial<NPChartParams>) =>
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "data" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            The NP chart plots the raw count of defective units. It is the P
            chart on a different scale, and it reads well only when the sample
            size is constant {"\u2014"} otherwise a count of 40 means one thing
            in a sample of 100 and another in a sample of 500.
          </p>
        </div>
      )}
      ...
      {tab === "params" && (
        <div className="space-y-3">
          <p className="text-xs text-gray-500">
            Enter a historical proportion to use instead of estimating it from
            the data. Leave blank to estimate.
          </p>
          ...
          <p className="-mt-1 text-xs text-gray-500">
            A proportion, not a count: the centre line is n {"\u00D7"} p, and it
            follows from the sample size.
          </p>
        </div>
      )}
      ...
      {tab === "limits" && (
        <div className="space-y-3">
          ...
          <p className="-mt-1 text-xs text-gray-500">
            The limits are always clipped to the interval 0 to n regardless of
            these bounds: a count of defective units cannot fall outside it.
          </p>
          ...
          <div className="border-t border-gray-200 pt-2">
            ...
            <p className="mt-1 text-xs text-gray-500">
              On an NP chart unequal sizes also step the centre line, not just
              the limits, which is what makes the chart hard to read. If the
              sizes vary at all, prefer the P chart.
            </p>
          </div>
        </div>
      )}
      ...
      {tab === "tests" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            ...
            zones that assume a symmetric distribution about the centre line,
            which the binomial is not unless p is close to 0,5.
          </p>
        </div>
      )}
      ...
      {tab === "stages" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            ...
            own centre line, and no run test carries across a boundary.
          </p>
        </div>
      )}
    </div>
