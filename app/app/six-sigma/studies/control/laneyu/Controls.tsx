// app/app/six-sigma/studies/control/laneyu/Controls.tsx
"use client";
import React, { useState } from "react";
import type { ColumnInfo } from "../../../lib/columns";
import ColumnSelect from "../../../components/ColumnSelect";
import type { LaneyUParams, SizeMode, TestMode } from "./types";
...
export default function LaneyUControls({
  params,
  onChange,
  columns,
}: {
  params: LaneyUParams;
  onChange: (p: LaneyUParams) => void;
  columns: ColumnInfo[];
}) {
  const [tab, setTab] = useState<Tab>("data");
  const set = (patch: Partial<LaneyUParams>) =>
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "data" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            Same data a U chart takes: the count is the number of{" "}
            <em>defects</em>, not of defective units, and the size is a number of
            inspection units that may be fractional. The U{"\u2032"} chart differs
            only in the width of its limits.
          </p>
          <p className="text-xs text-gray-500">
            Run the ordinary U chart too. If the two agree, the Poisson model is
            adequate and the simpler chart is the one to keep.
          </p>
        </div>
      )}
      ...
      {tab === "params" && (
        <div className="space-y-3">
          <p className="text-xs text-gray-500">
            Enter a historical rate to use instead of estimating it from the data.
      ...
      {tab === "limits" && (
        <div className="space-y-3">
          ...
          <p className="-mt-1 text-xs text-gray-500">
            ...
            unlike a proportion, a rate of defects has no ceiling {"\u2014"} which
            is why the widened upper limit can climb a long way at small inspection
            sizes.
          </p>
        </div>
      )}
      ...
      {tab === "tests" && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-500">
            ...
            {"\u2014"} nine points on one side {"\u2014"} does not care how wide
            the limits are, so widening them does not silence it. Sigma Z fixes
            false alarms of test 1, not run patterns.
          </p>
        </div>
      )}
