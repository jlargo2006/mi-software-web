// app/app/six-sigma/studies/ht/sign/Results.tsx
"use client";
...
import { ALT_SYMBOL, type HTSignParams, type HTSignResult } from "./types";
...
export default function HTSignResults({
  result,
  params,
}: {
  result: HTSignResult;
  ...
  const interval = (lo: number, hi: number): string =>
    r.ciKind === "two"
      ? `(${f(lo, 4)}; ${f(hi, 4)})`
      : r.ciKind === "lower"
        ? `(${f(lo, 4)}; ${INFTY})`
        ...
        <div className="w-full space-y-6">
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                <tr className="border-b border-gray-200">
                  ...
                  <td className="py-1">{f(r.sampleMedian, 4)}</td>
                </tr>
              </tbody>
            </table>
            {r.nMissing > 0 && (
          ...
          {r.performTest && (
            <section className="mb-6">
              ...
              <table className="border-collapse text-sm">
                <thead>
                  <tr className="border-b border-gray-300 text-left text-gray-600">
                    ...
                    <th className="py-1 pr-6">Number &lt; {f(r.eta0, 0)}</th>
                    <th className="py-1 pr-6">Number = {f(r.eta0, 0)}</th>
                    <th className="py-1 pr-6">Number &gt; {f(r.eta0, 0)}</th>
                ...
                <tbody>
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="py-1 pr-6">{r.nBelow}</td>
                    <td className="py-1 pr-6">{r.nEqual}</td>
                    <td className="py-1 pr-6">{r.nAbove}</td>
              ...
              {r.nEqual > 0 && (
                <p className="mt-2 text-xs text-amber-700">
                  {r.nEqual} observation(s) equal to the hypothesized median carry
                  no sign and were excluded from the test.
                </p>
              )}
            </section>
          )}
          ...
          {/* Confidence interval */}
          {r.performCI && r.rows.length > 0 && (
            <section className="mb-6">
              <h3 className="mb-2 text-sm font-semibold text-gray-800">
                {f(r.confLevel, 0)}% Confidence Interval for {ETA}
              </h3>
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="border-b border-gray-300 text-left text-gray-600">
                ...
                <tbody>
                  {r.rows.map((row, i) => (
                    <tr key={i} className="border-b border-gray-200">
                      <td className="py-1 pr-4">{i === 0 ? r.column : ""}</td>
                      <td className="py-1 pr-4">{interval(row.low, row.high)}</td>
                      <td className="py-1 pr-4">{f(row.conf, 2)}%</td>
                      <td className="py-1">
                        {row.interpolated
                          ? "Interpolation"
                          : row.posLow === null
