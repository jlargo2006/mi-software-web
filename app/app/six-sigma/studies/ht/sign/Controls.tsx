// app/app/six-sigma/studies/ht/sign/Controls.tsx
"use client";
import React from "react";
import type { ColumnInfo } from "../../../lib/columns";
import { ALT_LABEL, type SignAlternative, type HTSignParams } from "./types";
...
export default function HTSignControls({
  params,
  onChange,
  columns,
}: {
  params: HTSignParams;
  onChange: (p: HTSignParams) => void;
  columns: ColumnInfo[];
}) {
  const set = <K extends keyof HTSignParams>(k: K, v: HTSignParams[K]) =>
    onChange({ ...params, [k]: v });
    ...
    <div className="space-y-4">
        ...
        <div className={params.performTest ? "space-y-3" : "space-y-3 opacity-50"}>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => set("alternative", e.target.value as SignAlternative)}
              disabled={!params.performTest}
            >
              {(Object.keys(ALT_LABEL) as SignAlternative[]).map((k) => (
      ...
      <div className="border-t border-gray-200 pt-4 space-y-3">
        ...
        <div className={params.performCI ? "" : "opacity-50"}>
          ...
          />
          <p className="mt-1 text-xs text-gray-500">
            Order statistics are discrete: the report shows the two reachable
            intervals plus the interpolated one.
          </p>
        </div>
      </div>
